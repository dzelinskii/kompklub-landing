...
В Claude Design завершить макеты сайта-поезда (экстерьер, вид вагонов, раздвижная граффити-дверь, индикатор-схема; тёмная база + кислотно-зелёный, дозированный glow — см. бриф `docs/superpowers/claude-design-brief.md` и раздел 8 спецификации) и экспортировать handoff-бандл для Claude Code. Здесь же дорабатывается тонкая анимация двери и, при желании, навигация колесом/свайпом поверх готовой клавиатурной и кнопочной. Также довести до ума CTA внутри вагонов в режиме поезда: инлайн-кнопка «Предварительная регистрация» в hero-вагоне сейчас ведёт по якорю `#pre-register`, что корректно в компактной вертикали, но в горизонтальном поезде (вагоны смещены через transform) не прокручивает к форме — нужно, чтобы она, как CTA в шапке, переключала поезд на вагон контактов.
